...
class InstancePrinter extends CTemplate {
	...
	'''
		...
		«IF !actor.inputs.nullOrEmpty»
			...
			«FOR port : actor.inputs»
				...
				extern connection_t connection_«incomingPortMap.get(port).getSource().getLabel()»_«actor.label»;
				#define rate_«port.name» connection_«incomingPortMap.get(port).getSource().getLabel()»_«actor.label».rate				
				«ELSE»
				extern connection_t connection_«(port as Port).name»_«actor.label»;
				#define rate_«port.name» connection_«(port as Port).name»_«actor.label».rate				
				«ENDIF»
			«ENDFOR»
			«IF enableTrace»
				////////////////////////////////////////////////////////////////////////////////
	...
	def protected printActorScheduler() '''
		...
		«ELSE»
			«noInline»void «entityName»_scheduler(schedinfo_t *si) {
				...
				printf( "«entityName» scheduler starts ...\xD\xA" );
			...
			finished:
				...
				«IF actor.inputs.nullOrEmpty && actor.outputs.nullOrEmpty »
					...
					return;
				«ENDIF»
				printf( "«entityName» scheduler ends ...\xD\xA" );
			}
		«ENDIF»
	'''
	...
	override caseInstAssign(InstAssign inst) { 
		OrccLogger::warnln( "InstAssign: target type: "+inst.target.variable.type+"; value type: "+inst.value.type )
		if( inst.target.variable.type.string ) { 
			if( inst.value.type.string ) '''
				strcpy( «inst.target.variable.name» , «inst.value.doSwitch» );
	'''
			else
			if( inst.value.type.float ) '''
				strcpy( «inst.target.variable.name»,string_operand_float(«inst.value.doSwitch»)); 
	'''
			else
			if( inst.value.type.int ) '''
				strcpy( «inst.target.variable.name»,string_operand_int( «inst.value.doSwitch» )); 
	'''
		} else { '''
	...
	override caseInstLoad(InstLoad load) {
		...
		OrccLogger::warnln( "InstLoad: target type: "+load.target.variable.type+"; source type: "+load.source.variable.type )
		'''
			...
			«IF port !== null» ««« Loading data from input FIFO
				...
				«IF (isActionAligned && port.hasAttribute(currentAction.name + "_" + ALIGNABLE)) || port.hasAttribute(ALIGNED_ALWAYS)»
					«IF target.type.string»
						strcpy( «target.name»,tokens_«port.name»[(index_«port.name» % SIZE_«port.name») + («load.indexes.head.doSwitch»)] );
					«ELSE»
						«target.name» = tokens_«port.name»[(index_«port.name» % SIZE_«port.name») + («load.indexes.head.doSwitch»)];
					«ENDIF»
				«ELSE»
					«IF target.type.string»
						strcpy( «target.name»,tokens_«port.name»[(index_«port.name» + («load.indexes.head.doSwitch»)) % SIZE_«port.name»] );
					«ELSE»
			...
			«ELSE»
				...
				«ENDIF»
				«IF target.type.string && (source.type.string || ( source.type.list && (source.type as TypeList).getInnermostType().string))»
					strcpy( «target.name» , «source.name»«load.indexes.printArrayIndexes» );
				«ELSE»
	...
	override caseInstStore(InstStore store) {
		...
		OrccLogger::warnln( "InstStore: target type: "+store.target.variable.type+"; store value type: "+store.value.type )
		...
		'''
			...
			«IF port !== null» ««« Storing data to output FIFO
				...
				«ELSEIF (isActionAligned && port.hasAttribute(currentAction.name + "_" + ALIGNABLE)) || port.hasAttribute(ALIGNED_ALWAYS)»
					«IF target.type.string»
						strcpy( tokens_«port.name»[(index_«port.name» % SIZE_«port.name») + («store.indexes.head.doSwitch»)], «store.value.doSwitch» );
					«ELSE»
						tokens_«port.name»[(index_«port.name» % SIZE_«port.name») + («store.indexes.head.doSwitch»)] = «store.value.doSwitch»;
					«ENDIF»
				«ELSE»
					«IF target.type.string || ( target.type.list && (target.type as TypeList).getInnermostType().string )»
						strcpy( tokens_«port.name»[(index_«port.name» + («store.indexes.head.doSwitch»)) % SIZE_«port.name»], «store.value.doSwitch»);
					«ELSE»
			...
			«ELSE»
				...
				«ENDIF»
				«IF target.type.string && (store.value.type.string || ( store.value.type.list && (store.value.type as TypeList).getInnermostType().string))»
					«store.value.doSwitch»
					strcpy( «target.name»«store.indexes.printArrayIndexes»,"" );
				«ELSE»
